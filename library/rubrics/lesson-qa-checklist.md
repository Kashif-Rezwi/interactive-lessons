...
- [ ] **Repair re-render rule:** after *any* repair to a generated artifact (assembly recovery, restored function, typo fix), re-run the rendered pass and require zero page errors before trusting the repair. Static balance checks (`node --check`) prove syntax, not execution — a restored function can reference an undefined variable and still parse cleanly (RUN-20260924-0001: `dyg`/`dydg`).
...
| 2026-09-24 | Added Audit 6's **repair re-render rule**, after RUN-20260924-0001 recovered a long-artifact assembly failure whose restored functions passed `node --check` and presence greps while one still threw a runtime `ReferenceError` — caught only by the rendered pass (MEM-2026-0007). |
